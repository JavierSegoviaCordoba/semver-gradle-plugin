...
import com.javiersc.semver.shared.internal.git.isThereVersionTag
import com.javiersc.semver.shared.internal.git.lastVersionTagInCurrentBranch
import com.javiersc.semver.shared.internal.git.tagName
import com.javiersc.semver.shared.internal.git.tagsInCurrentBranch
import com.javiersc.semver.shared.internal.git.tagsInCurrentBranchHash
import com.javiersc.semver.shared.internal.git.tagsInCurrentBranchName
import com.javiersc.semver.shared.internal.git.tagsInCurrentBranchRef
import com.javiersc.semver.shared.internal.git.tagsInCurrentCommit
import com.javiersc.semver.shared.internal.git.tagsInRepo
import com.javiersc.semver.shared.internal.git.tagsInRepoHash
import com.javiersc.semver.shared.internal.git.tagsInRepoName
import com.javiersc.semver.shared.internal.git.tagsInRepoRef
import com.javiersc.semver.shared.internal.git.versionTagsInCurrentBranch
import com.javiersc.semver.shared.internal.git.versionTagsInCurrentBranchSortedByTimelineOrSemverOrder
import com.javiersc.semver.shared.internal.git.versionTagsInCurrentCommit
import com.javiersc.semver.shared.internal.git.versionTagsSortedBySemver
...
internal class GitTagTest {
    ...
    fun `tags in repo name`() {
        initialCommitAnd {
            ...
            git.tagsInRepoName.shouldBe(
                listOf("refs/tags/hello", "refs/tags/v100", "refs/tags/vhello")
            )
            ...
            git.tag().setName("v3.0.0").call()
            git.tagsInRepoName.shouldBe(
                listOf("refs/tags/hello", "refs/tags/v100", "refs/tags/v3.0.0", "refs/tags/vhello")
            )
    ...
    fun `tags in current branch`() {
        initialCommitAnd {
            ...
            git.tagsInCurrentBranch
                .map(GitRef.Tag::name)
                .shouldBe(listOf("hello", "v100", "vhello"))
            ...
            git.tagsInCurrentBranch
                .map(GitRef.Tag::name)
                .shouldBe(listOf("hello", "v1.0.0", "v100", "v3.0.0", "vhello"))
    ...
    fun `tags in current branch ref`() {
        initialCommitAnd {
            ...
            git.tag().setName("vhello").call()
            git.tag().setName("v100").call()
            git.tagsInCurrentBranchRef.map(Ref::tagName).shouldBe(listOf("hello", "v100", "vhello"))
    ...
    fun `tags in current commit`() {
        initialCommitAnd {
            ...
            git.tagsInCurrentCommit(git.headCommit.commit.hash)
                .map(GitRef.Tag::name)
                .shouldBe(listOf("v1.0.0", "v2.0.0", "v20.20", "v200"))
        }
    }
